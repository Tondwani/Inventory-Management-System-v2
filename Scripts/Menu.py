...
class InventoryHomeMenuDashboard:
    ...
    def create_header(self):
        ...
        tk.Label(self.header, text="Welcome!! Streamline your Inventory & Elevate your Efficiency", fg="white", bg="dark blue", font=("Gabriola", 13)).pack(anchor="center", expand=True)
        ...
        self.chatbot_button = tk.Button(self.header, text="EchoxAI", bg="yellow", command=None)
        self.chatbot_button.pack(side=tk.RIGHT, padx=10)
    ...
    def create_footer(self):
        self.footer_frame = tk.Frame(self.root, bg="dark blue")
        self.footer_frame.pack(side=tk.BOTTOM, fill=tk.X)
        self.title_label = tk.Label(self.footer_frame, text="©2024 Inventory Management System. All rights reserved.", fg="white", bg="dark blue", font=("Arial", 10))
        self.title_label.pack(pady=5)
        self.developer_info = tk.Label(self.footer_frame, text="Developed by Tondwani Craig", fg="white", bg="dark blue", font=("Arial", 10))
        self.developer_info.pack(pady=5)


    def create_hamburger_menu(self):
        self.menu_frame = tk.Frame(self.root, bg="white", width=200)
        self.menu_frame.place(x=-200, y=80, relheight=0.70)
        ...
        self.hamburger_button.place(x=10, y=70)
        ...
        for text, command in menu_items:
            button = tk.Button(self.menu_frame, text=text, command=command, bg="white", fg="black", bd=0, padx=10, pady=25, anchor="w", width=15)
    ...
    def create_main_content(self):
        ...
        self.main_frame.place(x=200, y=65, relwidth=0.8, relheight=0.9)

    def execute_query(self, query):
        self.cursor.execute(query)
        return self.cursor.fetchone()

    def create_info_box(self, parent, title, query, col, color):
        frame = tk.Frame(parent, bg=color, relief="ridge", bd=1)
        frame.grid(row=0, column=col, padx=5, pady=5, sticky="nsew")
        tk.Label(frame, text=title, font=("Arial", 10, "bold"), bg=color, fg="black").pack(pady=2)
        ...
        tk.Label(frame, text=formatted_value, font=("Arial", 9), bg=color, fg="black").pack()
    ...
    def update_dashboard(self):
        ...
        # Create info boxes
        info_queries = [
            ("Total Employees", "SELECT COUNT(*) FROM Employees", "#FF6B6B"),  # Red
            ("Total Suppliers", "SELECT COUNT(*) FROM Suppliers", "#4ECDC4"),  # Teal
            ("Total Sales", "SELECT SUM(TotalAmount) FROM Billing", "#45B7D1"),  # Blue
            ("Low Stock Items", "SELECT COUNT(*) FROM Products WHERE Quantity < 10", "#FFA07A"),  # Light Salmon
            ("Total Inventory Value", "SELECT SUM(Price * Quantity) FROM Products", "#98D8C8")  # Mint
        ]

        for i, (title, query, color) in enumerate(info_queries):
            self.create_info_box(info_row, title, query, i, color)
    ...
    def open_sales(self):
        ...
        SalesManager(sales_window, main_menu_callback=self.show_main_Menu)

    def open_categories(self):
        self.root.withdraw()  
        category_window = tk.Toplevel()
        from category import CategoryManager  
        CategoryManager(category_window, main_menu_callback=self.show_main_Menu)

    def open_billing(self):
        self.root.withdraw()  
        billing_window = tk.Toplevel()
        from billing import BillingManager  
        BillingManager(billing_window, main_menu_callback=self.show_main_Menu)

    def open_employees(self):
        self.root.withdraw()  
        employee_window = tk.Toplevel()
        from employee import EmployeeManager  
        EmployeeManager(employee_window, main_menu_callback=self.show_main_Menu)
        
    def open_products(self):
        self.root.withdraw()  
        product_window = tk.Toplevel()
        from product import ProductManager  
        ProductManager(product_window, main_menu_callback=self.show_main_Menu)
        
    def open_suppliers(self):
        self.root.withdraw()  
        supplier_window = tk.Toplevel()  
        from supplier import SupplierManager  
        SupplierManager(supplier_window, main_menu_callback=self.show_main_Menu)

    def open_settings(self):
        self.root.withdraw()  
        settings_window = tk.Toplevel()  
        from settings import SettingsManager  
        SettingsManager(settings_window, main_menu_callback=self.show_main_Menu)
