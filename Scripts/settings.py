...
class SettingsManager:
    def __init__(self, parent, main_menu_callback):
        ...
        self.initialize_user_sessions_table()
        self.update_user_activity("CurrentUser")
        self.setup_window()
        self.create_widgets()
        ...
        try:
            ...
            messagebox.showerror("Initialization Error", f"An error occurred during system initialization: {str(e)}")
    ...
    def setup_window(self):
        ...
        # Add header frame
        self.header_frame = tk.Frame(self.main_frame, bg='light grey')
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = tk.Label(self.header_frame, text="Settings", font=('Arial', 16, 'bold'), bg='light grey')
        ...
        self.datetime_label = tk.Label(self.header_frame, font=('Arial', 12), bg='light grey')
        self.datetime_label.pack(side=tk.RIGHT, pady=5, padx=10)
    ...
    def create_help_tab(self):
        ...
        4. For detailed stats, use the 'System Info' tab.
        5. Always backup your settings and data regularly.
        """
    ...
    def initialize_user_sessions_table(self):
        try:
            with sqlite3.connect(self.database_path) as conn:
                ...
                ''')
                conn.commit()
            logging.info("User sessions table initialized successfully")
        except sqlite3.Error as e:
            logging.error(f"An error occurred while initializing user_sessions table: {str(e)}")
            raise
    ...
    def update_user_activity(self, username):
        try:
            with sqlite3.connect(self.database_path) as conn:
                ...
                cursor.execute('''
                INSERT OR REPLACE INTO user_sessions (username, last_active)
                VALUES (?, ?)
                ''', (username, current_time))
                conn.commit()
            logging.info(f"User activity updated for user: {username}")
        except sqlite3.Error as e:
            logging.error(f"An error occurred while updating user activity: {str(e)}")
            raise
    ...
    def view_active_users(self):
        try:
            ...
            if active_users:
                user_info = "\n".join([f"{username} - Last active: {last_active}" for username, last_active in active_users])
                messagebox.showinfo("Active Users", user_info)
                self.status_bar.config(text=f"Viewing {len(active_users)} active users")
                logging.info(f"Viewed {len(active_users)} active users")
            else:
                messagebox.showinfo("Active Users", "No active users in the last 24 hours.")
                self.status_bar.config(text="No active users found")
                logging.info("No active users found in the last 24 hours")
        except sqlite3.Error as e:
            error_message = f"An error occurred while retrieving active users: {str(e)}"
            messagebox.showerror("Database Error", error_message)
            self.status_bar.config(text="Error retrieving active users")
            logging.error(error_message)

    def go_back(self):
        self.parent.destroy()  
        if hasattr(self, 'main_menu_callback') and self.main_menu_callback:
